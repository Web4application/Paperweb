...
#. If you are using the ``auth_type`` "config", it is suggested that you
   ...
   recommended, for example with HTTPâAUTH in a :term:`.htaccess` file or switch to using
...
Some additional downloads (for example themes) might be signed by Michal ÄihaÅ. His key id is
...
configured. This is called âZero Configurationâ mode and can be particularly
useful in shared hosting situations. âZeroconfâ mode is on by default, to
...
* Unlike cookie and http, does not require a user to log in when first
  ...
  incoming HTTP requests at oneâs router or firewall will suffice (both
